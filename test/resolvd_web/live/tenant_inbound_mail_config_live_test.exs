defmodule ResolvdWeb.TenantInboundMailConfigLiveTest do
  ...
  @update_attrs %{
    mailbox: "some updated mailbox",
    mark_when_read: false,
    password: "some updated password",
    port: 43,
    server: "some updated server",
    tls: false,
  ...
  describe "Index" do
    ...
    test "lists all tenant_inbound_mail_configs", %{
      conn: conn,
      tenant_inbound_mail_config: tenant_inbound_mail_config
    } do
      {:ok, _index_live, html} = live(conn, ~p"/tenant_inbound_mail_configs")

      assert html =~ "Tenant inbound mail configs"
    ...
    test "saves new tenant_inbound_mail_config", %{conn: conn} do
      ...
      assert index_live
             |> form("#tenant_inbound_mail_config-form",
               tenant_inbound_mail_config: @invalid_attrs
             )
    ...
    test "updates tenant_inbound_mail_config in listing", %{
      conn: conn,
      tenant_inbound_mail_config: tenant_inbound_mail_config
    } do
      {:ok, index_live, _html} = live(conn, ~p"/tenant_inbound_mail_configs")
      ...
      assert index_live
             |> element("#tenant_inbound_mail_configs-#{tenant_inbound_mail_config.id} a", "Edit")
             |> render_click() =~
               "Edit Tenant inbound mail config"

      assert_patch(
        index_live,
        ~p"/tenant_inbound_mail_configs/#{tenant_inbound_mail_config}/edit"
      )
    ...
    test "deletes tenant_inbound_mail_config in listing", %{
      conn: conn,
      tenant_inbound_mail_config: tenant_inbound_mail_config
    } do
      {:ok, index_live, _html} = live(conn, ~p"/tenant_inbound_mail_configs")
      ...
      assert index_live
             |> element(
               "#tenant_inbound_mail_configs-#{tenant_inbound_mail_config.id} a",
               "Delete"
             )
             |> render_click()
      ...
      refute has_element?(
               index_live,
               "#tenant_inbound_mail_configs-#{tenant_inbound_mail_config.id}"
             )
    end
  end
  ...
  describe "Show" do
    ...
    test "displays tenant_inbound_mail_config", %{
      conn: conn,
      tenant_inbound_mail_config: tenant_inbound_mail_config
    } do
      {:ok, _show_live, html} =
        live(conn, ~p"/tenant_inbound_mail_configs/#{tenant_inbound_mail_config}")

      assert html =~ "Show Tenant inbound mail config"
      assert html =~ tenant_inbound_mail_config.mailbox
    end

    test "updates tenant_inbound_mail_config within modal", %{
      conn: conn,
      tenant_inbound_mail_config: tenant_inbound_mail_config
    } do
      {:ok, show_live, _html} =
      ...
      assert_patch(
        show_live,
        ~p"/tenant_inbound_mail_configs/#{tenant_inbound_mail_config}/show/edit"
      )
